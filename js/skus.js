...
function buildSkuTable(array) {
    ...
    for (var k = 0; k < a.length; k++) {
        ...
        body += '<td><input type="hidden" class="skusid" data-props="' + sku_row_id + '" value="0" name="Item[skus][table][' + k + '][sku_id]">' +
        '<input label="tag" class="skus-tag" style="width: 90%" value="" type="text" data-id="' + sku_row_id + '" name="Item[skus][table][' + k + '][tag]"></td>' +
            '<td><input label="price" class="skus-price" value="" type="text" data-id="'+sku_row_id +'" name="Item[skus][table]['+k+'][price]"></td>' +
            '<td><input label="quantity" class="skus-qty" data-id="'+sku_row_id +'" value="" type="text" name="Item[skus][table]['+k+'][quantity]"></td>' +
            '<td><input label="outer_id" class="skus-outer_id" value="" data-id="'+sku_row_id +'" type="text" name="Item[skus][table]['+k+'][outer_id]"></td>' +
        '<td class="operation"><a href="javascript:void(0);" data-id="' + sku_row_id + '" ><i class="icon-cog"></i></a></td></tr>';
    }
...
function renderBatchLayer(optArr) {
    ...
    html.push('<div class="batch-type col-lg-6">');
    html.push('<div class="caption">标签：</div>');
    html.push('<ul class="list">');
    for (var i = 0; i < count; i++) {
        html.push('<li>');
        html.push('<input class="batch-radio" data-type="tag" data-index="' + i + '" name="batch-tag" data-optId="' + optArr[i][0] + '" id="batch-tag-' + i + '" type="radio">');
        html.push('<label for="batch-tag-' + i + '">同' + $("#thop_" + i).text() + '标签相同</label>');
        html.push('</li>');
    }
    html.push('</ul>');
    html.push('</div>');
...
function updateSkus(res) {
    ...
    $.each(json, function (i, data) {
        ...
        $("#sku").find("input[class='skus-tag'][data-id='" + data["props"] + "']").val(data["tag"]);//更新tag
    });
}
...
$(document).ready(function () {
    ...
    function apply_same_param(optid, index, type) {
        ...
        if (index == 0) {
            ...
            var cur_tag = $("#sku").find("input[class='skus-tag'][data-id='" + rid + "']").val();
            if (type == 0) {
                ...
                var oid = $("#sku").find("input[type=hidden][data-index='" + index + "']").data("id");
                $("#sku").find("input[class='skus-tag'][data-id*='" + optid + "']").each(function () {
                    $(this).val(cur_tag);
                });
            }
        } else {
            ...
            for (var i = 0, count = opAr.length; i < count; i++) {
                ...
                $("#sku").find("input[type=hidden][class='skusid'][data-props*='" + mat + "']").each(function () {
                    ...
                    cur_tag = tr.find("input[class='skus-tag']").val();
                    if (type == 0) {
                        ...
                        $("#sku").find("input[class='skus-tag'][data-id*='" + opMatch + "']").each(function () {
                            $(this).val(cur_tag);
                        });
                    }
                });
    ...
    $(document).on("click", "#btnPopSub", function (e) {
        ...
        $(".popover-content").find("input[type=radio][data-type=tag]:checked").each(function () {
            var optid = $(this).data('optid');
            var index = $(this).data('index');
