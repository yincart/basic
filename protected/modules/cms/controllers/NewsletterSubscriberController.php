...
//                    exit;
            ...
            $model->customer_id = 0;
            $model->email = $_POST['NewsletterSubscriber']['email'];
            $model->status = 0;
            $model->confirm_code = md5(time());
            $model->change_status_at = date('Y-m-d H:i:s');

            if ($model->save()) {
                echo '<script>alert("Success!")</script>';
                echo '<script type="text/javascript">setTimeout(\'location.href="' . Yii::app()->createUrl('/site/index') . '"\',10);</script>';
                die;
            } else {
